...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    let menu = { food: { coxinha: 5, sopa: 10 }, drink: { cerveja: 7, agua: 4 } };
    objetoRetornado = createMenu(menu);
    expect(objetoRetornado.fetchMenu()).toMatchObject(menu);
    ...
    expect(objetoRetornado.consumption).toEqual(expect.arrayContaining(['coxinha']));
    ...
    objetoRetornado = createMenu(menu);
    let orders = ['agua', 'coxinha', 'sopa', 'agua'];
    for (const item of orders) {
      objetoRetornado.order(item);
    }
    expect(objetoRetornado.consumption).toEqual(expect.arrayContaining(orders));
    // Agora faça o TESTE 7 deste arquivo.
    // --------------------------------------------------------------------------------------

    // TESTE 7: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    objetoRetornado = createMenu(menu);
    orders = ['agua', 'coxinha', 'sopa', 'agua'];
    for (const item of orders) {
      objetoRetornado.order(item);
    }
    expect(objetoRetornado.consumption).toEqual(expect.arrayContaining(orders));
    ...
    expect(objetoRetornado.pay()).toEqual(23);
  });
});
